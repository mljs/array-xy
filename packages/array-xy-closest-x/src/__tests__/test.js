...
describe('closestX', () => {
    ...
    it('should yield the correct result with odd element array and descending order', () => {
        ...
        expect(closestX({x, y}, {target: -2, reverse: true})).toEqual({x: -1, y: 10});
        expect(closestX({x, y}, {target: 0.6, reverse: true})).toEqual({x: 1, y: 12});
        expect(closestX({x, y}, {target: 4.3, reverse: true})).toEqual({x: 4, y: 15});
        expect(closestX({x, y}, {target: 6, reverse: true})).toEqual({x: 6, y: 17});
        expect(closestX({x, y}, {target: 7, reverse: true})).toEqual({x: 7, y: 18});
        expect(closestX({x, y}, {target: 8, reverse: true})).toEqual({x: 7, y: 18});
    });
});
